$(function() {
    $("nav#menu")
        .mmenu({
            ...
            navbars: [{
                height: 3,
                content: ['<a align="center"><img src="../../static/images/coupon_icon4.png" /></a><div class="bbiigg" id="username"><a href="admin">管理员</a></div>']
            }],
            counters: true,
                ...
                for (var i = 0; i < data.result.length; i++) {
                    if (data.result[i].actype === 1) {
                        if (data.result[i].finished === false) {
                            ...
                            item += '进行中';
                            item += '</span></h5></div><div class="about-text"><a href="admin_detail/';
                            ...
                            item1 += '已结束';
                            item1 += '</span></h5></div><div class="about-text"><a href="admin_detail/';
                            item1 += data.result[i].acid;
                            item1 += '"><h4 style="color:#919c9c">';
                            item1 += data.result[i].subject;
                            item1 += '</h4><p style="color:#919c9c">';
                ...
                for (var j = 0; j < data.result.length; j++) {
                    if (data.result[j].actype === 2) {
                        if (data.result[j].finished === false) {
                            ...
                            item2 += '进行中';
                            item2 += '</span></h5></div><div class="about-text"><a href="admin_detail/';
                            ...
                            item3 += '已结束';
                            item3 += '</span></h5></div><div class="about-text"><a href="admin_detail/';
